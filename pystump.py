...
from flask import Flask, g, render_template, request, json,  abort, redirect, session, url_for
...
def index():
    if g.db_corrupt:
        ...
        return render_template("main.jinja2", announcements=announcements, **g.std_args)

@app.route("/list")
@login_required
def list_announcements():
    announcements = g.db.get_all_announcements()
    return render_template("list.jinja2", announcements=announcements, **g.std_args)

@app.route("/edit")
@app.route("/edit/<id>")
def edit_announcement(id=None):
    announcement = g.db.get_announcement(id)
    return render_template("edit.jinja2", announcement=announcement, **g.std_args)
...
def login_page():
    ...
    if request.method == 'POST':
        ...
        auth = Authenticator(authenticators[app.config.get("AUTH_BACKEND")], **app.config.get("LDAP_CONFIG"))
...
def post(callback):
    ...
    if callback not in callbacks.keys() or not session.get("auth"):
        ...
        result = callbacks.get(callback)(request.form, username=session.get("username"))
